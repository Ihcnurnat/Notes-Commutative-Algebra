...
\end{align*}where the last inclusion is given by Exercise 1.18 on Page 10 of Atiyah's textbook\todo{Could we extend the original result to infinitely many primes?}. But the only way that an extended ideal to be zero is the ideal 
...
Counterexample. Let $A=\mathbb Q$ and $B=\mathbb C$ with $f$ being an inclusion. Then $f^{\ast}$ is injective for $B$ is a field. But $\langle i\rangle$ is not an extended ideal for $\langle i\rangle^{ce}=\langle 0\rangle ^e=0$.\todo{?}